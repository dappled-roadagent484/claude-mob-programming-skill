...
- **Cunningham** - 单元测试专家，负责编写测试方案和测试代码（很遗憾，Turing因多次违规擅自编写生产代码，被开除了）
...
你可以使用“启动pair programming团队，完成……任务”来启动结对编程团队。该团队由两位agent组成，一位负责实现，一位负责审查。也有可能一位负责写测试，一位负责写生产代码。根据场景智能选择模式。
```
